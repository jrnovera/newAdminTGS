-- =============================================================================
-- THE GLOBAL SANCTUM — WELLNESS MIGRATION v1
-- Extends the enterprise schema with wellness-specific fields.
--
-- Run AFTER schema_v2.sql in Supabase SQL Editor.
-- Safe to re-run (uses IF NOT EXISTS / OR REPLACE everywhere, plus DROP VIEW ... CASCADE before recreating).
-- =============================================================================

-- =============================================================================
-- STEP 1 — Extend existing satellite tables with wellness-specific columns
-- =============================================================================

-- venue_basic_info: wellness type tags, treatment rooms, practitioners
...
-- venue_content: second intro paragraph for wellness editorial
ALTER TABLE venue_content
    ADD COLUMN IF NOT EXISTS intro_text TEXT;

-- venue_amenities: wellness-specific amenity categories
...
-- venue_owner_info: extended owner/business contact
...
-- venue_internal: CRM / pipeline tracking
...
-- =============================================================================
-- STEP 2 — New table: venue_operating_hours (wellness hours per day)
-- =============================================================================
...
BEGIN
    IF NOT EXISTS (
        ...
        WHERE tablename = 'venue_operating_hours' AND policyname = 'Authenticated full access'
    ) THEN
    ...
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies
        WHERE tablename = 'venue_operating_hours' AND policyname = 'Public read operating hours'
    ) THEN
...
-- =============================================================================
-- STEP 3 — New table: venue_wellness_config
-- Pricing, services, facilities, and amenities editorial config for wellness venues
-- =============================================================================
...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies
        WHERE tablename = 'venue_wellness_config' AND policyname = 'Authenticated full access'
    ) THEN
    ...
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies
        WHERE tablename = 'venue_wellness_config' AND policyname = 'Public read wellness config'
    ) THEN
...
-- =============================================================================
-- STEP 4 — Recreate v_venues_full to include new tables and columns
-- Must DROP first because CREATE OR REPLACE cannot shift existing column positions.
-- =============================================================================
...
-- =============================================================================
-- STEP 5 — Grants for new tables and refreshed view
-- =============================================================================
