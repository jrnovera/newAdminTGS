...
export default function WellnessVenues() {
    ...
    // Filter venues to only include Wellness facilities, then apply tab/search filters
    const wellnessVenues = venues.filter(v => v.type === 'Wellness');

    const filteredVenues = wellnessVenues.filter((v) => {
        const matchesSearch =
            searchQuery === '' ||
    ...
    // Dynamic tab counts for Wellness
    const tabCounts = [
        wellnessVenues.length,
        wellnessVenues.filter((v) => v.status === 'Active').length,
        wellnessVenues.filter((v) => v.status === 'Draft').length,
        wellnessVenues.filter((v) => v.status === 'Inactive').length,
    ];
    ...
    return (
        <>
            ...
            <header className="page-header">
                <div>
                    <h1 className="page-title">Wellness Venues</h1>
                    <p className="page-subtitle">Manage all wellness facility listings across the platform</p>
                </div>
            ...
            <div className="toolbar">
                <div className="search-box">
                    ...
                    <input
                        type="text"
                        placeholder="Search wellness venues by name, location, or owner..."
                        value={searchQuery}
            ...
            <div className="data-table-container">
                        ...
                        {loading ? (
                            <tr>
                                <td colSpan={8} style={{ textAlign: 'center', padding: '48px 20px' }}>
                                    ...
                                    <div style={{ marginTop: 12, color: '#666' }}>Loading wellness venues...</div>
                                </td>
                            </tr>
                        ) : filteredVenues.length === 0 ? (
                            <tr>
                                <td colSpan={8} style={{ textAlign: 'center', padding: '48px 20px', color: '#B8B8B8' }}>
                                    <div style={{ fontSize: 16, marginBottom: 8 }}>No wellness venues found</div>
                                    <div style={{ fontSize: 13 }}>
                                        {searchQuery ? 'Try adjusting your search or filters' : 'Click "Add Wellness" to create your first venue'}
                                    </div>
                                </td>
                            </tr>
                        ) : (
                            filteredVenues.map((v) => (
                                <tr key={v.id} style={selectedIds.has(v.id) ? { backgroundColor: 'rgba(245, 158, 11, 0.06)' } : {}}>
                                    ...
                                    <td onClick={() => navigate(`/wellness-venues/${v.id}`)} style={{ cursor: 'pointer' }}>
                                        ...
                                        <div className="action-menu-wrapper" ref={openMenuId === v.id ? menuRef : null}>
                                            ...
                                            {openMenuId === v.id && (
                                                <div className="action-menu">
                                                    <button className="action-menu-item" onClick={() => { setOpenMenuId(null); navigate(`/wellness-venues/${v.id}`); }}>
                ...
                {filteredVenues.length > 0 && (
                    <div className="pagination">
                        <div className="pagination-info">Showing 1-{filteredVenues.length} of {filteredVenues.length} wellness venues</div>
